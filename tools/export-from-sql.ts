...
for (const post of posts.values()) {
	...
	if (!file) continue;
	attachmentPathById.set(post.id, `/img/${file}`);
}
...
function rewriteContent(html: string): string {
	...
	// Rewrite legacy upload paths to the new /img/ prefix
	out = out.replace(/\/wp-content\/uploads\//g, '/img/');

	// Track uploads referenced in the body
	for (const m of out.matchAll(/\/img\/[^\s'")<>]+/g)) {
...
// (d) Pre-launch SEO-audit fixups: WP URLs the rebuild dropped
// that don't fall out of the SQL export naturally.
//
// `/event/race-the-train/` was a single Tribe Events page in WP and
// has no analogue in the rebuild — the events listing took its
// place. The Redirection plugin rule that survives in source (b)
// only handles the `index.html` form and points at a non-existent
// destination, so we need the canonical `/<slug>/` form here.
//
// `/category/uncategorized/` is the WP default term archive that
// Yoast had x-robots-tag: noindex on, so it has minimal equity
// anyway. 410 it cleanly so search engines don't keep crawling.
redirects.push(
	{ from: '/event/race-the-train/', to: '/events/', code: 301, source: 'seo-audit' },
	{ from: '/event/race-the-train/index.html', to: '/events/', code: 301, source: 'seo-audit' },
	{ from: '/category/uncategorized/', to: '/404.html', code: 410, source: 'seo-audit' },
);

// Deduplicate (later-source wins). The 'seo-audit' fixups come
// last so they overwrite any broken Redirection-plugin rule with
// the same `from`.
...
// Netlify-style _redirects format: `/from /to <code>`. Specific
// rules are sorted alphabetically; wildcards must come last so
// they don't shadow more specific matches.
